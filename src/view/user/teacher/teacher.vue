 ...
 <Card shadow>
    <div class="teacher">
        <Form ref="formValidate"  :model="formValidate"  :label-width="80" inline>
            <FormItem label="讲师名称" prop="nickname">
                <Input clearable  v-model="formValidate.nickname" placeholder="请输入讲师关键字"></Input>
            </FormItem>
        ...
        <div>
          <Button type="primary"  icon="md-add"  class="btn-margin-right" @click="$router.push({name: 'teacherEdit'})">新增讲师</Button>
        </div>
        <div class="content">
          <Table :loading="loading" stripe  border ref="selection" :columns="columns" :data="teacherList">
            ...
            <template slot="action" slot-scope="{ row }">
                <Button type="text" size="small" class="btn-text" @click="$router.push({name:'teacherEdit', query:{id: row.id}})">编辑</Button>
                <span>-</span>
...
export default {
  ...
  methods: {
    deletHandle (row) { // 删除
      this.$Modal.confirm({
        title: '提示',
        content: '<p>删除后，该讲师将无法登录讲师端，且讲师列表将不再展示该讲师信息，与之关联的数据也将被清除，是否确认？</p>',
        onOk: () => {
